...
public class CancelJobAction extends BaseAction {
	...
	private static Logger log = Logger.getLogger(CancelJobAction.class);
		...
		try {
			...
			if ( jobId > 0 ) {
				...
				try {
					...
					GUICallStatus status = connToServer.cancelJob( jobId, dbRecordVersionNumber );
					...
					} else if ( status == GUICallStatus.FAILED_JOB_NO_LONGER_CANCELABLE ) {

						ActionErrors errors = new ActionErrors();
						errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("error.job.no.longer.cancelable"));
						saveMessages(request, errors);
