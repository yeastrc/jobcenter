 ...
 */
@Path("/postToJCServer")
public class RequeueJobSendToServerService {

	private static final Logger log = Logger.getLogger( RequeueJobSendToServerService.class );

	@POST
	@Consumes( MediaType.APPLICATION_FORM_URLENCODED )
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/requeueJob") 
	public RequeueJobResponse requeueJob(   
	...
	throws Exception {
		try {
			...
			GUICallStatus guiCallStatus =
					connToServer.requeueJob( jobId, jobRecordVersionId );

			RequeueJobResponse webserviceResponse = new RequeueJobResponse();
			...
			if ( guiCallStatus ==  GUICallStatus.SUCCESS ) {
				...
				log.error( "Unknown Status returned from 'connToServer.requeueJob', guiCallStatus = " + guiCallStatus );
			}
	 ...
	 *
	 */
	public static class RequeueJobResponse {
